...
its rotational invariance.

\paragraph{Loss Function}

All models are optimized using the $\ell_1$ reconstruction loss
...
All experiments are conducted on a GPU-enabled environment when available.
Training is performed on a CUDA-compatible device, with automatic fallback to
CPU execution if GPU resources are unavailable. This setup ensures efficient
training while maintaining reproducibility across different hardware
configurations.
